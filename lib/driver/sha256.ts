...
export const sha256 = function(m) {
  ...
  (new SHA256()).update(decode(m)).finish(h).clean();
  return hexEncode(uint8ToString(h));
};

export const hmac = function(k, m) {
  var h = new Uint8Array(32);
  (new HMAC(decode(k))).update(decode(m)).finish(h).clean();
  return hexEncode(uint8ToString(h));
};
